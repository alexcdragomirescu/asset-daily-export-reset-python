...
logging.info("Couting pre-reset elements...")
val_count['a']['export'].update(
    count_property(
        os.path.join(export, 'a')
    )
)
...
logging.info("Couting post-reset elements...")
val_count['a']['reset'].update(
    count_property(
        os.path.join(export, 'a')
    )
)
...
for i in val_count:
    for j in val_count[i]:
        total = 0
        for k in val_count[i][j]:
            total += val_count[i][j][k]
        val_count[i][j].update({'total': total})

global_count = {'reset': sum(val_count[i]['reset']['total'] for i in val_count), 'export': sum(val_count[i]['export']['total'] for i in val_count)}
...
logging.info("Authoring admin email body...")
...
logging.info("Authoring user email body...")
templateLoader = jinja2.FileSystemLoader(searchpath=templates)
templateEnv = jinja2.Environment(loader=templateLoader)
...
ssh.command('''python /export/home/taskexec/send_mail/mail.py \
        ...
        --to 'EDITED' \
        --to 'EDITED' \
        --subject 'DAILY_EXPORT Flag Reset on {event_time}' \
